...
void term_update_resp_egr_fw(hls::stream<egrFwUpdateResp> &update_resp,
                             hls::stream<egrFwLookupResp> &read_resp,
...
#pragma HLS INTERFACE mode = ap_ctrl_none port = return
    ...
    else if (!read_resp.empty())
    {
        ...
        tmp |= term_resp.value.connection;
        tmp |= ((axilite_data_t)term_resp.value.ifid) << CONNECTION_WIDTH;
        ...
        tmp |= ((axilite_data_t)term_resp.value.virt) << FW_TABLE_REG_FLAG_VIRTUAL;
        tmp |= ((axilite_data_t)term_resp.value.blocking) << FW_TABLE_REG_FLAG_BLOCKING;
        resp_data = tmp;
